...
import org.fossify.messages.dialogs.EditContactKeyDialog
...
class ThreadActivity : SimpleActivity() {
    ...
    private fun refreshMenuItems() {
        ...
        val hasSharedSecret = E2eManager.hasSharedSecret(this, threadId)
        binding.threadToolbar.menu.apply {
            ...
            findItem(R.id.exchange_keys).isVisible = canUseE2e
            findItem(R.id.encrypt_messages).isVisible = canUseE2e
            findItem(R.id.encrypt_messages).isEnabled = hasSharedSecret
    ...
    private fun sendKeyExchange() {
        ...
        val subscriptionId = availableSIMCards.getOrNull(currentSIMCardIndex)?.subscriptionId
            ?: SmsManager.getDefaultSmsSubscriptionId()
        val message = E2eManager.buildKeyExchangeMessage(this)
    ...
    private fun toggleEncryption(menuItem: MenuItem) {
        if (!E2eManager.hasSharedSecret(this, threadId)) {
            toast(R.string.e2e_no_shared_key)
            return
        }
        val enable = !menuItem.isChecked
        E2eManager.setThreadEncrypted(this, threadId, enable)
        menuItem.isChecked = enable
        toast(if (enable) R.string.e2e_encryption_enabled else R.string.e2e_encryption_disabled)
    }

    private fun editContactKey() {
        val contact = participants.firstOrNull() ?: return
        if (contact.rawId <= 0 && contact.contactId <= 0) {
            return
        }
        val currentKey = E2eManager.getContactPublicKey(this, contact)
        EditContactKeyDialog(this, currentKey) { newKey ->
            if (newKey.isNullOrBlank()) {
                E2eManager.removeContactPublicKey(this, contact)
                E2eManager.clearSharedSecret(this, threadId)
                E2eManager.setThreadEncrypted(this, threadId, false)
            } else {
                E2eManager.storeContactPublicKey(this, contact, newKey)
                E2eManager.ensureSharedSecretFromContact(this, threadId, contact)
                E2eManager.setThreadEncrypted(this, threadId, true)
            }
            refreshMenuItems()
            toast(R.string.e2e_contact_key_saved)
        }
    }
    ...
    private fun setupButtons() = binding.apply {
        ...
        binding.messageHolder.apply {
            ...
            threadSendMessage.setOnLongClickListener {
                if (!isScheduledMessage && !isMeshThread()) {
            ...
            if (alarmManager.canScheduleExactAlarms()) {
                ...
                PermissionRequiredDialog(
                    ...
                    positiveActionCallback = {
                        openRequestExactAlarmSettings(packageName)
                    },
                )
            }
        } else {
    ...
    private fun setupParticipants() {
        ...
        participants.firstOrNull()?.let { contact ->
            E2eManager.ensureSharedSecretFromContact(this, threadId, contact)
        }
    }

    private fun isMeshThread(): Boolean {
        if (LxmfAddress.isMeshThreadId(threadId)) return true
        val intentNumber = intent.getStringExtra(THREAD_NUMBER)
        if (!intentNumber.isNullOrBlank() && LxmfAddress.isMeshAddress(intentNumber)) return true
        val conversationNumber = conversation?.phoneNumber
        if (!conversationNumber.isNullOrBlank() && LxmfAddress.isMeshAddress(conversationNumber)) return true
        return participants.any { contact ->
            contact.phoneNumbers.firstOrNull()?.value?.let { LxmfAddress.isMeshAddress(it) } == true
        }
    }
    ...
    private fun sendMeshMessage(text: String) {
        val addresses = participants.getAddresses()
        ...
        if (threadId != meshThreadId) {
            threadId = meshThreadId
    ...
    fun refreshMessages(@Suppress("unused") event: Events.RefreshMessages) {
        ...
        val meshThread = isMeshThread()
        val newThreadId = if (meshThread) {
            threadId
        } else {
            getThreadId(participants.getAddresses().toSet())
        }
    ...
    private fun updateMessageType() {
        if (isMeshThread()) {
            binding.messageHolder.threadSendMessage.setText(R.string.mesh_message)
            return
        }
