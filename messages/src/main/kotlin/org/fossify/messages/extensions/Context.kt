...
fun Context.getThreadParticipants(
    ...
    if (LxmfAddress.isMeshThreadId(threadId)) {
        ...
        val address = conversation?.phoneNumber
        if (!address.isNullOrEmpty()) {
            val contact = MeshContactHelper.getSimpleContactForMeshAddress(this, address)
                ?: SimpleContact(
                    rawId = 0,
                    contactId = 0,
                    name = address,
    ...
    if (LxmfAddress.isMeshThreadId(threadId)) {
        ...
        if (conversation != null) {
            conversationsDB.insertOrUpdate(
                conversation.copy(read = true, unreadCount = 0)
            )
        } else {
...
fun Context.markThreadMessagesUnread(threadId: Long) {
    if (LxmfAddress.isMeshThreadId(threadId)) {
        ...
        if (conversation != null) {
            conversationsDB.insertOrUpdate(
                conversation.copy(read = false, unreadCount = maxOf(1, conversation.unreadCount))
            )
        } else {
...
fun Context.getThreadId(address: String): Long {
    if (LxmfAddress.isMeshAddress(address)) {
        return LxmfAddress.threadIdForAddress(address)
    }
    return try {
...
fun Context.getThreadId(addresses: Set<String>): Long {
    val meshAddress = addresses.firstOrNull { LxmfAddress.isMeshAddress(it) }
    if (meshAddress != null) {
        return LxmfAddress.threadIdForAddress(meshAddress)
    }
    return try {
...
fun Context.getNameFromAddress(address: String, privateCursor: Cursor?): String {
    if (LxmfAddress.isMeshAddress(address)) {
        val (name, _) = MeshContactHelper.getContactNameAndPhotoForMeshAddress(this, LxmfAddress.normalize(address))
...
fun Context.getContactFromAddress(address: String, callback: ((contact: SimpleContact?) -> Unit)) {
    if (LxmfAddress.isMeshAddress(address)) {
        callback(MeshContactHelper.getSimpleContactForMeshAddress(this, LxmfAddress.normalize(address)))
        return
    }
...
fun Context.updateLastConversationMessage(threadId: Long) {
    if (LxmfAddress.isMeshThreadId(threadId)) {
        LxmfStore.updateConversationFromDb(this, threadId)
        return
    }
...
fun Context.updateLastConversationMessage(threadIds: Iterable<Long>) {
    val meshThreads = threadIds.filter { LxmfAddress.isMeshThreadId(it) }
    meshThreads.forEach { LxmfStore.updateConversationFromDb(this, it) }
    val smsThreads = threadIds.filterNot { LxmfAddress.isMeshThreadId(it) }
    if (smsThreads.isEmpty()) return
